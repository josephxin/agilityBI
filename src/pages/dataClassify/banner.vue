<template>
	<div class="ify_vessel">
		<div class="ify_name">
			<p>
				{{list.name}}
				<i class="iconfont icon-bi opacity" @click="open"></i>
			</p>
			<i class="delete_banner iconfont icon-unie639 opacity" @click="deleteBanner(list)"></i>
		</div>
		...
		watch: {
			list: {
				handler(newValue, oldValue) {
					...
					//					this.commonClick(newValue.list.length,Math.ceil(newValue.list.length/3)-1)
					//					if(newValue.list.length<= 3){
